import math
import numpy as np
import convertTremuriToOpensees
import time
import pandas as pd
import subprocess
import os
...
def get_entries():
    ...
    projectName = prjNameBtn.get()
    batchFileName = batFileNameBtn.get() + ".bat"
    tclFileName = tclFileNameBtn.get() + ".tcl"
    freeVibrationTime = float(frVibTimeBtn.get())
    massDistribution = md.get()
    macroelementType = melt.get()
    dropDrift = float(drDr.get())
    ignoreDrift = igDr.get()
    pDelta = pDlt.get()
    pushoverPattern = pushPat.get()
    flexureShells = flxShll.get()
    root.destroy()


def get_directory(fileType):
    global TCLfilePath, OUTPUTfilePath, motionFilePath
    ttl = "Choose folder for " + fileType + " files"
    if fileType == "TCL":
        TCLfilePath = tk.filedialog.askdirectory(initialdir=os.getcwd(), title=ttl) + "/"
    elif fileType == "output":
        OUTPUTfilePath = tk.filedialog.askdirectory(initialdir=os.getcwd(), title=ttl) + "/"
    elif fileType == "Ground Motion":
        motionFilePath = tk.filedialog.askdirectory(initialdir=os.getcwd(), title=ttl) + "/"


root = Tk()
root.title("Tremuri - OpenSees Interface")

distributions = ["Tremuri", "Consistent", "Standard", "Lumped"]
md = StringVar(root)
md.set(distributions[2])  # default distribution
elementTypes = ["Tremuri", "Standard"]
melt = StringVar(root)
melt.set(elementTypes[0])  # default element type
pushoverPatterns = ["Tremuri", "Standard"]
pushPat = StringVar(root)
pushPat.set(pushoverPatterns[1])  # default pushover pattern type
igDrOpts = [True, False]
igDr = BooleanVar(root)
igDr.set(igDrOpts[0])
pDltOpts = [True, False]
pDlt = BooleanVar(root)
pDlt.set(pDltOpts[0])
flxShllOpts = [True, False]
flxShll = BooleanVar(root)
flxShll.set(pDltOpts[1])

Label(root, text="Project Name").grid(row=0, column=0)
Label(root, text="Batch File Name").grid(row=1, column=0)
Label(root, text="TCL File Name").grid(row=1, column=2)
Label(root, text="Free Vibration Time").grid(row=2, column=0)
Label(root, text="Standard Drop Drift").grid(row=2, column=2)
Label(root, text="Mass Distribution").grid(row=3, column=0)
Label(root, text="Macroelement Type").grid(row=3, column=2)
Label(root, text="Ignore Drift? (1:Y  0:N)").grid(row=4, column=0)
Label(root, text="Consider P-Delta? (1:Y  0:N)").grid(row=4, column=2)
Label(root, text="Pushover Pattern").grid(row=5, column=0)
Label(root, text="Flexure Shells? (1:Y  0:N)").grid(row=5, column=2)

prjNameBtn = Entry(root, width=66)
batFileNameBtn = Entry(root)
tclFileNameBtn = Entry(root)
frVibTimeBtn = Entry(root)
frVibTimeBtn.insert(10, 0)
drDr = Entry(root)

prjNameBtn.grid(row=0, column=1, columnspan=3)
batFileNameBtn.grid(row=1, column=1)
tclFileNameBtn.grid(row=1, column=3)
frVibTimeBtn.grid(row=2, column=1)
drDr.grid(row=2, column=3)
distrDrpDwn = OptionMenu(root, md, *distributions).grid(row=3, column=1)
meltDrpDwn = OptionMenu(root, melt, *elementTypes).grid(row=3, column=3)
igDrDrpDwn = OptionMenu(root, igDr, *igDrOpts).grid(row=4, column=1)
pDltDrpDwn = OptionMenu(root, pDlt, *pDltOpts).grid(row=4, column=3)
pushPatDrpDwn = OptionMenu(root, pushPat, *pushoverPatterns).grid(row=5, column=1)
flxShllDrpDwn = OptionMenu(root, flxShll, *flxShllOpts).grid(row=5, column=3)


Button(root, text='Apply', command=get_entries).grid(row=6, column=3, sticky=W, pady=4)
Button(root, text='TCL DIR', command=lambda: get_directory("TCL")).grid(row=6, column=0, sticky=W, pady=4)
Button(root, text='Output DIR', command=lambda: get_directory("output")).grid(row=6, column=1, sticky=W, pady=4)
Button(root, text='Record DIR', command=lambda: get_directory("Ground Motion")).grid(row=6, column=2, sticky=W, pady=4)
mainloop()

startTime = time.time()

# assign the required objects to variables
impNode = convertTremuriToOpensees.node.copy(deep=True)
impWall = convertTremuriToOpensees.wall.copy(deep=True)
impElement = convertTremuriToOpensees.element.copy(deep=True)
impMaterial = convertTremuriToOpensees.material.copy(deep=True)
impFixConstraint = convertTremuriToOpensees.fixconstraint.copy(deep=True)
impW2wConstraint = convertTremuriToOpensees.w2wconstraint.copy(deep=True)
impF2wConstraint = convertTremuriToOpensees.f2wconstraint.copy(deep=True)
impPolygon = convertTremuriToOpensees.polygon.copy(deep=True)
impAnalysis = convertTremuriToOpensees.analysis.copy(deep=True)
